...
class customer:
  def __init__(self, customer_id):
    ...
    # gender of customer:
    self.gender = random.sample( ['male','female'], k=1 )[0]
...
# keep a history of which product categories the customer has been shown, and which categories they have bought:  
...
# drop the temporary column 'TEMP_COL_money_spent':
traindat.drop( ['TEMP_COL_money_spent'], axis=1 )

```

```{python}
test = traindat.loc[ traindat['customer_id'].isin([6,69]) ][['customer_id','y_bought','TEMP_COL_money_spent']].copy()

test['historic_max_spend'] = test.groupby('customer_id').TEMP_COL_money_spent.shift(1).fillna(0).astype(int)
test['historic_max_spend'] = test.groupby('customer_id').historic_max_spend.cummax()
#test['test'] = test['TEMP_COL_money_spent']
#test.loc[ test['TEMP_COL_money_spent']==0, 'test' ] = 999999
test
```
...
py$traindat %>% 
  ...
  summarise( n_products_shown = n(),
             n_products_bought = sum(y_bought),
             .groups = "drop"
           ) %>% 
  group_by( n_products_shown, n_products_bought ) %>% 
