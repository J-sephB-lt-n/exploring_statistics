...
def get_deep(
    ...
    doc_max = max(data[multi_hot_columns].max())
    ...
    multihot_emb_avg = tf.keras.backend.mean(multihot_emb_stack, axis=2)
...
def get_wide_and_deep(
    ...
    columns = one_hot_columns + numerical_columns + multi_hot_columns
    ...
    train_ds, valid_ds = dataset.get_test_and_train(columns, df=data)
