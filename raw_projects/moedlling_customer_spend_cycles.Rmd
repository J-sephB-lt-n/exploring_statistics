...
Some customers buy in cycles. For example, a customer might regularly replenish a particular cosmetics product. Is there a simple way to model this behaviour using on the transaction times? 
...
Now, let's see how well a cycle of 3 fits to this data:
...
This doesn't seem like a very good fit. We can measure the *purity* of each cycle (i.e. cycle contains mostly the same value) using a metric such as *variance*, *entropy* or *gini*:
...
We could use the average (mean) value of one of these metrics as a measure of the goodness-of-fit of a fit that we're considering: 
...
Proceeding with the *gini* metric, let's try different cycle lengths and period lengths and see how low an average *gini* value we can get for this data:
...
# Compare models to different datasets
## model comparison test 1

```{r}
sim_data <- c(0, 25 ,42 ,133 ,232 ,330 ,353 ,407 ,533 ,595 ,631 ,645 ,707 ,743 ,812 ,959 ,1044 ,1113)

plot_transactions_ftn( spend_times = sim_data )  

# iterative period/cycle grid method------------------------------------------------------
find_best_model_by_gini <- find_best_model_ftn(   period_lengths_to_try = 10:50,
                                           cycle_lengths_to_try = 2:6,
                                                    spend_times = sim_data,
                                                         metric = "mean_gini"
                                )

best_model_by_gini <- find_best_model_by_gini %>% arrange(avg_metric) %>% slice(1)

fit_n_cycle_ftn(             spend_times = sim_data,
                 spend_times_period_form = 
                   assign_spend_times_to_periods_ftn( spend_times = sim_data,
                                            time_units_per_period = best_model_by_gini$period_length,
                                                             plot = FALSE
                                                    ),
                                  time_units_per_period = best_model_by_gini$period_length,
                                           cycle_length = best_model_by_gini$cycle_length,
                                                   plot = TRUE,
                                   n_periods_to_predict = 8
              )

# gap clustering method:
run_gap_clustering <- gap_clustering_model_ftn(     spend_times = sim_data,
                                                     seq_length = 2,
                                                k_values_to_try = 2:4
                                              )
run_gap_clustering <- gap_clustering_model_ftn(     spend_times = sim_data,
...
```{r}
sim_data <- c( 0 ,93 ,270 ,295 ,365 ,582 ,622 ,668 ,745 ,768 ,788 ,954 )
...
## model comparison test 3
```{r}
sim_data <- c( 0 ,84 ,126 ,289 ,329 ,455 ,520 ,679 ,765 ,878 ,905 ,1017 ,1031 ,1109 )
...
sim_data <- c( 0, 141, 162, 221, 229, 344, 358, 490, 570, 585, 609, 809, 872, 970, 1034, 1100, 1103, 1137 )
